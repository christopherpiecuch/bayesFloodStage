%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   function bayes_main_code_paper_lorocv
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Code written by CGP 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function bayes_main_code_paper_lorocv(regionNo)
...
%%%
% regions
load('20240602_taskteam_regions.mat','NOAAID','Region')
myreg(8)={'Alaska'};
myreg(7)={'Northwest'};
myreg(6)={'Southwest'};
...
nameOfExperiment='floodstagepaper_lorocv';            % experiment name
save_name=[nameOfExperiment,'_',char(myreg(regionNo))];  % file name
...
% delete entry for leave-one-out cross validation
if regionNo>9|regionNo<1
    error('invalid region number. must be between 1 and 9')
else
    for kk=numel(lat):-1:1
        ijkl=find(id(kk)==NOAAID);
        if strcmp(char(char(Region(ijkl))),char(myreg(regionNo)))
            lat(kk)=[];
            lon(kk)=[];
            nws(kk)=[];
            nos(kk)=[];
...
for nn=1:NN
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    V_ALPHA=[]; PSI_ALPHA=[];
    V_ALPHA=HP.eta_tilde_alpha/HP.zeta_tilde_alpha_2+ONE_N'*invOmegaMat*(u-beta*ell);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% V_BETA=[]; PSI_BETA=[];
    %%% V_BETA=HP.eta_tilde_beta/HP.zeta_tilde_beta_2+ell'*invOmegaMat*(u-alpha*ONE_N);
   	%%% PSI_BETA=inv(1/HP.zeta_tilde_alpha_2+ell'*invOmegaMat*ell);
